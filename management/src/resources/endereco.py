...
from src.utils.funcoes_auxiliares import atualizar_objeto, retorno_nao_autorizado
...
class EnderecosResource(MethodResource, Resource):
    ...
    def post(self, **kwargs):
        ...
        if cliente:
            ...
            empresa_id = FuncionarioModel.encontrar_empresa_id_por_funcionario_id(usuario_id)
...
class EnderecoResource(MethodResource, Resource):
    ...
    def put(self, **kwargs):
        ...
        if str(portador_id) == get_jwt_identity():
            ...
            if endereco.salvar():
                resposta = make_response(
                    endereco_schema.dump(endereco), 201
                )
...
class EnderecosClienteResource(MethodResource, Resource):
    ...
    def get(self, **kwargs):
        ...
        enderecos_retorno = []
        enderecos = EnderecoModel.listar_enderecos_por_entidade(kwargs['id'], 'cliente')
...
class EnderecosEmpresaResource(MethodResource, Resource):
    ...
    def get(self, **kwargs):
        resposta = make_response({'message': 'Endereços não encontrados'}, 404)
        enderecos_retorno = []
        enderecos = EnderecoModel.listar_enderecos_por_entidade(kwargs['id'], 'empresa')
